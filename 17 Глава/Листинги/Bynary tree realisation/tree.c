...
bool AddItem(const Item *pi, Tree *ptree) {
    Trnode *new_node;
    if (TreeIsFull(ptree)) {
        fprintf(stderr, "Tree is crowded\n");
        return false;
    }

    if (SeekItem(pi, ptree).child != NULL) {
        fprintf(stderr, "Trying of adding double element\n");
        return false;
    }

    new_node = MakeNode(pi);
    if (new_node == NULL) {
        fprintf(stderr, "Cant creaet node\n");
        return false;
    }
...
static void AddNode(Trnode *new_node, Trnode *root) {
    if (ToLeft(&new_node->item, &root->item)) {
        ...
        fprintf(stderr, "Error of position in AddNode()\n");
        exit(1);
    }
}
