...
void doing_actions(struct seat seats[], int size) {
    ...
    while (ch != 'f') {
        ...
        switch (ch) {
            ...
            case 'e':
                freeseat(is_reservateds, first_names, second_names, size);
                break;
            ...
            case 'f':
                ...
                for (int i = 0; i < size; ++i) free(first_names[i]), free(second_names[i]);
                free(first_names), free(second_names);
                break;
            
            // Wrong realisation of option g, don't choose while testing!
            case 'g':
                confirmate(is_reservateds, first_names, second_names);
                break;
