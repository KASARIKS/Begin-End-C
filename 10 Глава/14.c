...
double count_average_all(double[N], double);
double find_max(double[N][M], double);
...
int main(void) {
    ...
    average = count_average_all(averages, average);
...
double count_average_all(double averages[N], double average) {
    average = 0;
    for (int i = 0; i < N; ++i) average += averages[i];
    return average /= N;
}

double find_max(double arr[N][M], double max) {
    max = arr[0][0];
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
